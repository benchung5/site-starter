...
class GridView extends Component {
    ...
    renderItems() {
     return this.props.results.trees.map((item) => {
         return (
            <a href={`/filter_plants/view/${item.slug}`} className="product-card" alt={item.common_name} key={item.id} data-slug={item.slug}>
                <div className="inner">
                    <div className="image">
                        { item.images ?
                        <picture>
                        ...
                        :
                        <picture>
                            <source srcSet="/assets/img/placeholder-images/placeholder-img.jpg" media="(max-width: 1275px)"/>
